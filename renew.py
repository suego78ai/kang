...
modal_match = re.search(r'<!-- Submission Details Modal -->.*?</div>\s*</div>\s*</div>\s*</div>', content, re.DOTALL)
modal_html = modal_match.group(0) if modal_match else ''

dashboard_match = re.search(r'<!-- 실시간 통계 요약.*?(?=</section>)', content, re.DOTALL)
dashboard_html = dashboard_match.group(0) if dashboard_match else ''
idx = dashboard_html.rfind('</div>')
if idx != -1: dashboard_html = dashboard_html[:idx+6]
